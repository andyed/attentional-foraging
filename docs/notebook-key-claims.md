...
The K1–K27 rows are the stored **absolute, unbuffered, gaze-dependent** LAB
analysis. Their distance fields use gaze–cursor distance at fixation times;
proximity dwell sums fixation durations while the cursor is near the result.
They are not the browser's vertical cursor–AOI-distance aggregates. M3 also
includes explicit gaze dwell. Both legacy M3 and M4 include nine approach
fields, including `final_dist` and `retreat_dist`.

The K-bbox rows below record a separate May organic-attribution LAB retrain.
Their historical producer output path is no longer present at the cited name;
the available renamed diagnostic file is not numerically interchangeable.
These rows are **historical and pending source reconciliation**, not a current
post-AOI rerun. No historical K-ID is renumbered or promoted to the current
cursor-only stream. The original execution cells below still load the absolute
feature file; they do not execute the organic K-bbox analysis.

### 2026-05-01 attribution shift — historical LAB account
...
### Historical bbox-attribution K-IDs (gaze-dependent; source reconciliation pending)
...
> **Historical interpretation from the May organic LAB retrain; not a current recommendation.** The AR brand claim — "9 task-model-derived cursor features reach AUC ≈ 0.86 in LOSO" — survives and tightens under bbox. The position coefficient strengthens (−0.130 → −0.248): rank is a cleaner predictor when ads aren't pooled with organic. M1 AUC jumps from 0.613 to 0.727, which is paper-relevant: it means even position-alone is highly predictive once organic-rank is properly defined.
>
...
| ID | Claim | Value |
|---|---|---|
| **K1** | Records / participants / click rate | **13,419 per-(trial, position) records / 47 participants / 16.6 % click rate** (2,228 clicks) |
| **K2** | Records per participant | **median 294, range 76–497** |
| **K3** | M3 (position + gaze dwell + approach) mean participant LOSO AUC | **0.859 ± 0.044** (47-fold) |
...
>
> **Historical producer: `scripts/nb21_loso_retrain_organic.py`.** The cited `scripts/output/aoi-consumer-cascade/nb21_loso_organic.json` is absent in the current checkout. This script reads the gaze-dependent LAB file and must not be used as a cursor-only headline producer. Reconcile this lineage before refreshing the K-bbox values.
