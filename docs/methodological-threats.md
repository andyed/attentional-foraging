...
- **Cohort-size confound dominating the *p*-value.** At threshold = 50 px, the cohort is *smaller* than the canonical 100 px split (914 vs 2,355 records) and the *p*-value is *weaker* (10⁻¹³ vs 10⁻³⁸) — and the class medians (243 / 122 px) are directionally consistent with the canonical split (235 / 91 px). The *p*-value tracks *N* as expected. We report no confidence intervals on the per-threshold medians so cannot make an effect-size-stability claim beyond "directionally stable across all six sweep points."

**What this does NOT rule out:**

- **K7 (dwell_in_proximity_ms) is partially frozen, and its across-threshold "robustness" is partly a labeling artifact.** The "proximity" radius is baked into the feature at NB15 compute time at 100 px — so the K7 sweep above is only re-labeling records into deferred/rejected sets at each *approach* threshold, not re-computing proximity dwell at each *proximity* radius. A full K7 sweep would require regenerating `cursor-approach-features.json` at each proximity radius, which is out of scope here. Because K7 values are fixed per record, its across-threshold stability partly reflects records being re-sorted between buckets, not independent evidence. **Treat the K7 row of the sweep table as partially dependent on K5/K6, not as a third independent confirmation.** K5 (retreat_dist) and K6 (total_dwell_ms) are genuinely threshold-independent features; K7 is not.
