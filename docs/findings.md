...
**Typed current values (re-derived 2026-08-30: collision-fixed maps)** `[LAB, AdSERP, typed, NB22:K-typed-1..8]`: clicked **2,740 (14.1 %)**, deferred **3,300 (17.0 %)**, evaluated-rejected **578 (3.0 %)**, not-approached **12,770 (65.9 %)** of 19,388 records; motor dissociation deferred vs eval-rejected — post-closest drift **202 vs 53 px** (*p* = 3.0 × 10⁻⁵⁵), gaze dwell **3,055 vs 1,240 ms** (*p* = 2.6 × 10⁻⁸³), proximity dwell **1,096 vs 638 ms** (*p* = 1.5 × 10⁻¹⁹). NB22's in-notebook regression labels now come from the script-produced typed cache (`compute_regression_labels.py --attribution typed`); the notebook's four-class shares match the script producer exactly.
