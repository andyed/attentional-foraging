...
> **2026-09-14 / 09-15.** The five engagement states (never on screen / peripherally sampled vs unsampled / rejected / deferred / clicked), continuation measured five ways, the return mechanism, what the periphery does, the stutter step by position and the survey against the block above the fold are written up on their own page, [`findings-result-evaluation-states.md`](findings-result-evaluation-states.md), from post-fix producers, with Key Claims rows in NB37 and NB38 ([`notebook-key-claims.md`](notebook-key-claims.md)). Sections below keep their numbering so external links survive; §3b-ii, §3e and §10b carry pointers where the new producers qualify them.

> **Substrate re-derivation notice (2026-08-28 / 2026-08-30).** The typed / typed_gapfill AOI substrate was re-derived twice in late August 2026: the **2026-08-28 y-DP geometric realignment** and the **2026-08-30 AOI card-collision fix** (corpus 2,776 → **2,764 trials**, 12 alignment-excluded; see `docs/local-pack-aoi-shift.md`). Typed-flavor numbers in this file are being updated section-by-section with `(re-derived 2026-08-30: collision-fixed maps)` annotations; an unannotated typed value predates the passes and should be checked against [`docs/notebook-key-claims.md`](notebook-key-claims.md) before citing. `organic` / `organic_hybrid` values are unaffected by design.

**Status:** v15, 2026-05-04 — **typed AOI cascade landed.** All 11 Tier-A notebooks (NB04, NB14, NB15, NB18, NB22, NB23, NB24, NB25, NB28, NB30, NB32) re-executed under HTML+vision typed AOI map (`feat/aoi-pipeline-v3-typed`). NB21 click-prediction LOSO retrained: M3 AUC = **0.871** under typed; **0.870** under organic_hybrid — within fold-CI overlap (per-participant SD ≈ 0.044). 13 stress-test scripts also re-run under typed (within-item paired LF/HF return Δ = +6.44 vs hybrid +6.31, *p* < 10⁻²² in both). Per-etype click rates now visible across 9 etypes: organic 15.9 %, dd_top 17.1 %, paa 7.6 %, image_pack 6.6 %, knowledge_panel 5.8 %, native_ad 5.1 %, top_places 4.2 %. Each notebook's Key Claims block now carries a "## 2026-05-04 typed cascade" subsection with typed values; legacy (organic / organic_hybrid) K-IDs preserved. **Per-section prose update of this file is pending** — narrative values cited below reflect organic_hybrid (the prior cascade primary); the typed-vs-hybrid envelope is documented in the typed-cascade null-revisit table (see [`docs/null-findings/2026-05-04-typed-cascade-null-revisit.md`](null-findings/2026-05-04-typed-cascade-null-revisit.md), summarized as "qualitative story preserved across the 14 nulls + 2 recomputes; magnitude shifts on the order of ≤ 0.05 in correlation strength on the cases compared head-to-head"). See [`docs/notebook-key-claims.md`](notebook-key-claims.md) and [CHANGELOG 2026-05-04](../CHANGELOG.md) for the typed-attribution K-ID values. v14 (2026-04-13): §10b NB22 K5 metric renamed and reinterpreted: "Retreat distance" → "Post-closest drift". v13: §0 ski-jump satisfice/optimize decomposition retracted. v12 §0 ski-jump rebuilt on cohort A only. v11 Key Claims refs + stale value corrections.
...
**1. Saccade direction does NOT separate the phases (corrected 2026-06-03 — supersedes an earlier "survey reads across / evaluate reads down" claim).** On scroll-aware saccades the two phases are roughly balanced in orientation: survey 44.7% vs evaluate 48.7% primarily-vertical [NB13:K13/K14] — if anything evaluate is marginally *more* vertical. The earlier directional reading was a coordinate artifact: AdSERP fixations are page-space (scroll included), so differencing two fixations across a scroll event folds page motion into the saccade and inflates apparent verticality (the pre-correction page-space value was evaluate 51.2%; the prior clamped value flattened it the other way to 39.2%). See [`docs/methodology/scroll-aware-saccades.md`](methodology/scroll-aware-saccades.md). What *does* separate the phases is **amplitude**: survey saccades are wider on both axes (mean horiz/vert 99.7/82.6 px vs 71.1/71.9 px [NB13:K16/K17]; 1.54× median amplitude, K7) and more often major (53.0% vs 34.8% > 100 px [NB13:K18/K19]). The survey is wide gist-sampling, not a directional scan — consistent with the multi-cycle `f_scan_farce` result: the F-pattern is a measurement artifact, not a scan shape.
...
**5. What the five fixations are spent on (added 2026-09-15, `scripts/survey_above_fold.py`, [NB38:K10–K11]).** Under 1 % of five-fixation surveys reach the fold. On the 78 % of pages topped by an ad block (native_ad + dd_top) the first band fixated is the ad in 93 % of trials, the block takes 3.5 of the 5 fixations (fixation share 71 % against a 46 % share of above-fold height), and skipping over it to the first organic happens in 3 %; every participant does this (per-participant rate median 0.98, min 0.78). Only 13 % of the block's dwell falls inside the survey and 93 % of trials return to it. On pages with no ad block the same five fixations go to the organics (59 %) and the page top (20 %) in about their size shares. The "wide gist-sampling" of item 1 is therefore a description of how the *top block* is scanned, whatever it is; the periphery's part in it is local next-fixation guidance ([`findings-result-evaluation-states.md`](findings-result-evaluation-states.md), [NB37:K10–K11]).
...
## 3b-iv. Per-position LF/HF declines monotonically with rank — under the computation-dominated archetype, this reads as declining effort, not WM overload

Duchowski (2026) Butterworth IIR method enables per-result LF/HF measurement at the ~2-second per-position granularity where wavelet LHIPA cannot operate (minimum 7.5s). Two 4th-order Butterworth filters (LF: 0–1.6 Hz, HF: 1.6–4 Hz) applied to the full blink-cleaned 150 Hz pupil stream; LF/HF variance ratio computed per position during forward scanning.

**Polarity anchor (Duchowski 2026 §2.2).** Pupillary LF/HF polarity is task-archetype-dependent: in *computation-dominated* tasks (active criterion comparison, binary attend/skip judgments) more effort produces higher LF/HF; in *information-processing / memory-recall* tasks more effort produces lower LF/HF. SERP evaluation is computation-dominated — see `notebook-key-claims.md` § NB14 "Polarity convention" and `ettac-paper/sections/adserp.tex:43`. **For this dataset, higher LF/HF = more effort; the load interpretation below is conditional on that archetype assignment.** HRV-style polarity reasoning does not transfer (same name, different physiology).

**Empirical (polarity-agnostic).** LF/HF *declines* monotonically with rank (**ρ = −0.927, p < 0.0001** on *N* = 11 position medians aggregated from 2,719 trials) [NB14:K3]. The decline is steep through positions 0–3, then plateaus at approximately half the initial level through positions 4–10. **Interpretation under the computation-dominated archetype:** cognitive effort peaks at position 0 and drops steeply, ruling out the working-memory accumulation prediction (which expects effort to *rise* with rank). Under the alternative information-processing archetype, the same data would read as load *increasing* — that's why the archetype assignment in §3b-iv's polarity anchor is load-bearing.
...
Within-trial: median ρ = −0.400, **61.0 % of 1,025 trials show LF/HF declining with rank** [NB14:K5] — polarity-agnostic empirical fact. Under the archetype anchor above, this reads as declining effort within trial. Cross-validation: trial-mean LF/HF anti-correlates with LHIPA (ρ = −0.125, p = 7.47 × 10⁻¹⁰, N = 2,416) [NB14:K7]. LHIPA's polarity is fixed by construction (lower LHIPA = more load); the negative correlation means LF/HF and LHIPA agree on direction *under the computation-dominated archetype*, providing convergent validity that is partly independent of the archetype assignment itself.
...
**Connection to priming (§2).** The declining-effort profile (under the archetype anchor) is superficially consistent with lexical priming (repeated tokens easier to process). But priming was null at four granularities, and the LF/HF decline happens within the first 3 positions before substantial overlap accumulates. Framework compilation — not lexical facilitation — is the parsimonious explanation.
...
20.2% of fixations (39,442 of 195,468) fall within 48 px (~1.1° visual angle; corrected 2026-09-14 from the 17-inch 1707FP pixel pitch at the tracker's 65 cm ideal distance — the earlier "~2°" assumed 24 px/°) of the next result's boundary — the vertical parafoveal preview zone. These "margin fixations" are geometrically more likely to saccade to result N+1 (23.4% vs 6.1%), but the preview benefit test is null:
...
**The return at the gaze grain (added 2026-09-15, `scripts/next_action_by_position.py` and `scripts/return_is_memory.py`, [NB38:K1–K3, K7–K9]).** The "episodes" above are the stutter step seen from the result's side. From the second result down, the first visit to a result ends with a move back in 50–57 % of cases at every position, landing on a result already examined 75–92 % of the time; forward by one is second (32–38 %) and skipping two or more is 6–13 %. At result 2, three quarters of those excursions return to result 2 before any new result is entered. The return itself is executed from memory: long returns (≥ 2 ranks, 19 % of 9,347) land at first-entry precision from 2.4× the distance with a weaker peripheral ramp. The deferred state is the ordinary path through a result, not the exception, and the majority-back rate is a gaze-level fact that does not depend on scrolling (§4).
...
**A resumed pass is not a second survey.** Testing whether the epoch-2 scan re-enters the Survey phase — the amplitude signature, not the fixation index — the resumed pass opens at 76.0 px against Survey's 108.2 px, recovering 20.9 % [95 % CI 16.7–24.7] of the survey↔evaluate gap, with only 54.0 % [95 % CI 51.7–56.2] of trials rebounding individually (`scripts/epoch_saccade_amplitude.py`, `[LAB, AdSERP, organic_hybrid]`). What survives is a small decaying re-orientation cost at pass resumption, gone by epoch 3. Full write-up: [`docs/null-findings/2026-07-25-survey-reentry-on-regression.md`](null-findings/2026-07-25-survey-reentry-on-regression.md).
...
- **Revisit behavior is asymmetric.** Clicked results get +15% more fixations and +20% more time on revisit (confirmation). Non-clicked results get −18% fewer fixations (quick rejection). Per-fixation duration barely moves on revisit (clicked 216→226 ms; non-clicked flat ~212 ms). [LAB, AdSERP, typed_gapfill; `scripts/output/revisit_fixations_by_click/summary.json`, regression-split construct, 2026-08-30. Retired same date: the former +32%/+37%/−17% figures were prose-only (no K-ID, no executed cell reproduces them) and NB07b cell 10 used the retracted pre-2026-04-12 coordinate convention; the "duration drops slightly (210 vs 216ms) — recognition" sub-claim also did not survive. Episode-construct robustness row: clicked +4.1%, non-clicked −9.5%.]
...
**Typed current values (re-derived 2026-08-30: collision-fixed maps)** `[LAB, AdSERP, typed, NB22:K-typed-1]` through `[LAB, AdSERP, typed, NB22:K-typed-8]`: clicked **2,740 (14.1 %)**, deferred **3,300 (17.0 %)**, evaluated-rejected **578 (3.0 %)**, not-approached **12,770 (65.9 %)** of 19,388 records; motor dissociation deferred vs eval-rejected — post-closest drift **202 vs 53 px** (*p* = 3.0 × 10⁻⁵⁵), gaze dwell **3,055 vs 1,240 ms** (*p* = 2.6 × 10⁻⁸³), proximity dwell **1,096 vs 638 ms** (*p* = 1.5 × 10⁻¹⁹). NB22's in-notebook regression labels now come from the script-produced typed cache (`compute_regression_labels.py --attribution typed`); the notebook's four-class shares match the script producer exactly.
...
**The four classes become five states (added 2026-09-15, `scripts/engagement_state_census.py`, [NB37:K1–K4, K7]).** The cursor taxonomy's "not approached" and the gaze's "never fixated" are not one class. On the typed map with an opportunity baseline, 24 % of 34,317 result slots were never on screen, and of the 2,465 on-screen never-fixated slots a quarter received as much near-peripheral intake (within 200 px, ≈ 5°) per second of unfixated screen time as the results the searcher went on to read (per-participant median 0.27, IQR 0.20–0.37). The CHIIR carve's 660 approached-never-fixated rows split 168 peripherally sampled / 374 unsampled / 62 opportunity unknown. Examination cost then runs in tiers, median gaze dwell 0 / 541 / 1,695 / 4,052 ms for peripheral / rejected / deferred / clicked, with the cursor approaching 27 / 41 / 69 / 94 % of each. Every content cue is null across the peripheral tier, so it is a fact about sampling, not evaluation ([`findings-result-evaluation-states.md`](findings-result-evaluation-states.md)).

**Element-type cost surprise** `[absolute legacy, NB16; bbox re-derivation pending]`. Top ads show the highest pupil dilation during evaluation (+0.41%), contradicting the prediction that ads are cheaper to evaluate. Users may work harder to discriminate ads from organic results (notebook 16). Post-cascade hybrid attribution surfaces a complementary finding directly from click rates: **dd_top has the highest click rate of any SERP surface, well above organic and native_ad** `[LAB, AdSERP, organic_hybrid, NB21:K-bbox-* etype breakdown]` (per-etype counts and rates: [`docs/methodology/attribution-cascade-synthesis.md` §4.1](methodology/attribution-cascade-synthesis.md)) — directionally consistent with discrimination cost outweighing reading cost.

**Element-type approach signatures from motor data** `[LAB, AdSERP, typed, NB20]` *(re-derived 2026-08-30: collision-fixed maps)*. Top ads have 2× the approach rate (62.5% vs 31.4% organic), ~160 px more retreat among approached-and-rejected (mean 337 vs 175 px, p = 9.3×10⁻⁷⁷), 2.8× longer dwell in proximity for clicked results (6,847 vs 2,442 ms, p = 2.7×10⁻⁶²), and more direction changes (mean 2.8 vs 1.6 among approached, p = 3.8×10⁻³⁴). All effects survive position controls (positions 0–2: retreat p = 4.4×10⁻³⁷). Azzopardi's C/W/L cost framework (SIGIR 2018) is overturned for top ads — discrimination cost outweighs reading cost. Native ads show avoidance: lowest approach rate (25.5%) and lowest click rate (5.0%). The qualitative story of the absolute-legacy version (2× approach, more retreat, longer deliberation, native-ad avoidance) is preserved with stronger separation under typed attribution.
...
## 10c. The 4-fixation visual budget for first-pass SERP evaluation

Under bbox-organic attribution (post-2026-05-01 cascade), the per-(trial, organic position) predictor that most strongly separates will-regress from no-regress on first-pass visit is the simple count of fixations on that position before the user moves on. Median **5 fixations** on positions the user will return to vs **4 fixations** on positions the user has decided to skip. Mann–Whitney *p* = 3.3 × 10⁻¹⁶, *d* = +0.134, *N* = 8,844 records (6,332 wr / 2,512 nr) over 2,649 trials [NB18 / cascade-audit, `scripts/will_return_predictor_scan.py`]. The full survey of 14 candidate per-fixation predictors lives in [`docs/methodology/attribution-cascade-synthesis.md` §4.3](methodology/attribution-cascade-synthesis.md).

The number reads cleanly as a deployment-facing benchmark: the median user spends about 4 fixations on a SERP listing they'll skip; listings the user keeps the option open on earn 5+. The *will-regress* class includes both eventual clicks (deferred-then-clicked) and eventual skips (re-evaluated-then-rejected), so the 5-fixation threshold separates "kept option open" from "decided no" rather than "clicked" from "skipped." Distributions overlap heavily; the 4 vs 5 split is at the median of two distributions, not a hard cutoff. First-pass visit specifically — not lifetime fixation count.

The same scan resolves the R1 RIPA2 collapse (collapsed under bbox, *p* = 0.80) by surfacing per-fixation pupil replacements that survive cleanly: peak within-fixation dilation (`pd_change_max`, *p* = 5.1 × 10⁻⁶), peak constriction (`pd_change_min`, *p* = 1.9 × 10⁻⁵), entry-pupil baseline (`first_pd`, *p* = 5.2 × 10⁻⁷). Will-regress positions show *more* pupillary excursion (peaks ± larger) and lower entry pupil — a cleaner cognitive-engagement story than the legacy "lingered + processed shallowly" reading. Mean-based per-fixation pupil metrics (RIPA2 itself, `pd_change_mean`) all collapse the same way, suggesting the legacy R1 RIPA2 result was a rank-pooling artifact at the per-fixation amplitude level. Documented at [`docs/null-findings/r1-ripa2-bbox-collapse.md`](null-findings/r1-ripa2-bbox-collapse.md).

**Notebooks / scripts:** [`will_return_predictor_scan.py`](../scripts/will_return_predictor_scan.py), [`saccade_k_by_etype.py`](../scripts/saccade_k_by_etype.py), `scripts/output/aoi-consumer-cascade/will_return_predictor_scan.json`.
...
**Significance.** The ad-utility axis is the first observable, behaviorally-grounded substrate for the rank-value-prior framing used to interpret pupillary load (Anderson rational-analysis tradition). For the methods paper, it is a candidate premise: per-participant ad-utility prior should map onto consideration-set composition (clicked / deferred / evaluated-rejected / not-approached) in interpretable ways.
