...
Key Claims rows for every number on this page: `[NB37:K1–K15]` (`notebooks-v2/37_engagement_states.ipynb`) and `[NB38:K1–K11]` (`notebooks-v2/38_moves_between_results.ipynb`), aggregated in [`notebook-key-claims.md`](notebook-key-claims.md).
...
One more signature, from the fixation *before* a jump ([`ablations/major_saccade_selection.md`](ablations/major_saccade_selection.md)): major saccades follow shorter fixations, 154 ms before jumps over 600 px against 200 ms before minor ones, in 38 of 47 participants (producer [`scripts/major_saccade_selection.py`](../scripts/major_saccade_selection.py)), the ambient-mode timing of layout-driven scanning; and far jumps land on the nearest not-yet-read candidate more often than chance (0.24 against 0.17), though never more often than plain recency-proximity predicts, so the kernel adds nothing there either. Landing precision is flat across amplitude and prior intake does not improve it.
...
**The survey and the block above the fold.** The five-fixation survey does not route around the ad block that tops 78 % of these pages; it reads into it ([`ablations/survey_above_fold.md`](ablations/survey_above_fold.md); producer [`scripts/survey_above_fold.py`](../scripts/survey_above_fold.py)). On ad-topped pages the first band fixated is the ad in 93 % of trials, the block takes three and a half of the five fixations, and skipping over it to the first organic happens in 3 %; every participant does this. Nor is it a glance: 13 % of the block's dwell falls inside the survey and 93 % of trials come back to it. Under 1 % of surveys reach the fold, so the survey phase describes how the top block is scanned, whatever the block is.

**The sequence the states arise in.** Conditioning on the first fixation on a result ([`ablations/next_action_by_position.md`](ablations/next_action_by_position.md); producer [`scripts/next_action_by_position.py`](../scripts/next_action_by_position.py)): from the second result down, the first visit ends with a move back in 50–57 % of cases at every position, landing on a result already examined 75–92 % of the time; forward by one is second at 32–38 %; skipping two or more is 6–13 %. At result 2 three quarters of the back excursions return to result 2 before any new result is entered, a two-result comparison loop; deeper, the excursion widens to results the survey skipped and to the click. Result 1 is the exception: its first visit is twice as long and ends at the query box 44 % of the time. The deferred state is therefore the ordinary path through a result, not a special case, and the figure [`idealized_vs_population.png`](../scripts/output/figures/idealized_vs_population.png) sets one idealized trial beside the population rate of each of its moves.
