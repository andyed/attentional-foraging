"""Re-train NB21's M1/M2/M3/M4 LOSO classifiers on cursor-approach-features-typed-gapfill.

Mirrors nb21_loso_retrain_typed.py but on the typed_gapfill feature set
(midpoint-split bboxes + X+Y bbox-aware click attribution + 158 hard-error
trials filtered). Reports headline AUC/AP/Brier deltas vs typed legacy.

Regime tag: [LAB, AdSERP, typed_gapfill, NB21]
See: docs/null-findings/2026-05-05-bbox-y-coverage.md

Run:
    .venv/bin/python scripts/nb21_loso_retrain_typed_gapfill.py
"""
...
print("\n" + "=" * 70)
print(f"NB21 K-hybrid values (typed attribution, n={len(records):,} records)")
print("=" * 70)
...
# Save full result
out_path = ROOT / "scripts/output/aoi-consumer-cascade/nb21_loso_typed.json"
