"""Insert Key Claims markdown blocks at position [1] of cited notebooks.

Usage:
    python _add_key_claims.py

Re-runnable: if a "## Key Claims" cell already exists in the notebook, it
is replaced. Otherwise a new cell is inserted immediately after the title
cell (or at position 0 if the first cell isn't a title).

The canonical numbers are hand-curated from executed notebook output. Each
row is tagged with a stable ID (K1, K2, ...) so papers can cite
[NB13:K2] as a checkable reference.
...
Contract for paper writers:
    ...
    produces different values, update the Key Claims block immediately and
