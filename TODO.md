...
- [ ] **Practiced-participant effect audit.** Participants completed ~60 trials each. The 0ms orientation, crisp survey→evaluate transition, and high regression rate may reflect practiced behavior, not naive search. Audit all findings for practice sensitivity: (1) Does orientation time decrease across the first 10 trials? (2) Does survey duration tighten? (3) Do regression rates change? (4) Does saccade amplitude slope steepen with practice? If phase structure sharpens with practice, our findings describe the asymptotic expert pattern — which is what power users and repeat searchers do in production, but not what first-time visitors do. This caveat needs to appear in every generalizability discussion (explainer, CIKM, ETTAC, findings.md).
